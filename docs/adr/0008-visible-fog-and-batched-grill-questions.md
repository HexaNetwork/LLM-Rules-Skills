...
- Seed the register from `ReflectOutput.unknowns` at the reflect gate, so it is populated before the first question is asked. On structured confirm, re-seed from the operator's edited `unknowns` so draft entries they removed do not carry into grilling.
