...
Accepted; supersedes the shared-working-tree portion of [ADR 0009](0009-operational-intervention-and-budgets.md)
...
- **Create worktrees at run start, detached at an immutable `baseSha`.** Resolve the selected local base branch once, reserve `<stateRoot>/worktrees/<safeRunId>`, and run `git worktree add --detach <path> <baseSha>`. Persist versioned `workspace.json` (`RunWorkspace`) before agents or indexing touch the tree. Detached `HEAD` is intentional; the registered worktree keeps that tip reachable while the run exists.
