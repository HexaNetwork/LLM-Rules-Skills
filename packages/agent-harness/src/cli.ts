...
export function createCli(): Command {
  ...
  program.command("start").requiredOption("--project <id>").requiredOption("--idea <text>").option("--workflow <id>", "workflow", "complete").option("--base-branch <name>", "base branch").option("--fresh").action(async (value) => print(await api(program, "/api/runs", { method: "POST", body: { projectId: value.project, idea: value.idea, workflowId: value.workflow, baseBranch: value.baseBranch, fresh: Boolean(value.fresh) } })));
