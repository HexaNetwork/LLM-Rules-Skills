...
export const renderSettingsScript = `    function renderSettings(settings) {
      ...
      var fields = categories.map(function (category) {
        var rows = category.definitions.map(function (definition) {
          ...
          var applies = 'Applies to new runs; blocked runs use a reviewed recommended repair';
      ...
      var persistence = settings.editable
        ? 'Settings apply to new runs. Blocked runs use a reviewed recommended repair.'
      ...
      var artifactGroup = '<section class="settings-group" id="ignoredArtifactsGroup"><h3>Ignored build artifacts</h3>' +
        '<p class="faint" style="margin:0 0 8px">Harness-local globs skipped for dirty-tree and unreported-path checks. Does not edit .gitignore; use a recommended repair for a blocked run when needed.</p>' +
        '<div id="ignoredArtifactsList" style="display:grid;gap:8px">' + artifactRows + '</div></section>';
      $("settingsBody").innerHTML = '<p class="settings-intro">Tune workflow behavior from one place. Settings freeze into new runs; blocked runs can use an explicit reviewed repair.</p>' + fields + artifactGroup + displayGroup;
